    ...
    private func createLayers(image: UIImage!) {
        ...
        for i in 0 ..< 5 {
            ...
            line.path = {
                ...
                //                path.move(path, nil, lineFrame.midX, lineFrame.midY)
//                path.move(to: CGPoint(x: lineFrame.midX, y: lineFrame.midY))
//                path.add Path(CGPoint(x: lineFrame.origin.x + lineFrame.width / 2, y: lineFrame.origin.y) as! CGPath )
...
//        self.addTarget(self, action: "touchUpInside:", for: UIControlEvents.touchUpInside)
        self.addTarget(self, action: #selector(animationButton.touchUpInside(sender:)), for: .touchUpInside)
...
//        self.addTarget(self, action: "touchCancel:", for: UIControlEvents.touchCancel)
    ...
    func touchDown(sender: animationButton) {
        self.layer.opacity = 0.4
    }
    func touchUpInside(sender: animationButton) {
        self.layer.opacity = 1.0
    }
    func touchDragExit(sender: animationButton) {
        self.layer.opacity = 1.0
    }
    func touchDragEnter(sender: animationButton) {
        self.layer.opacity = 0.4
    }
    func touchCancel(sender: animationButton) {
        self.layer.opacity = 1.0
    }
